...
class OsticketConfig extends Config {
	...
	function updateModuleSettings($vars, &$errors) {
		...
		if(strcmp($vars['init_ticket_status'], 'on')==0 || strcmp($vars['update_status'], 'on')==0){
		  ...
		  $build = $vars['curbuild'];
		  $ver = $vars['curver'];
		  $current = $this->get('mod_status_sysbuild');
		 		  
		  //start from the oldest install and move through all updates based on current system version (saved in database)
		  switch($current){
			default:
				//First Install - Initial Setup
				$sql="CREATE TABLE ".MOD_STATUS."(id INT(64) NOT NULL AUTO_INCREMENT, PRIMARY KEY(id), statusName VARCHAR(64), colorBG VARCHAR(64), colorFG VARCHAR(64), colorBD VARCHAR(64), active BOOLEAN DEFAULT 1)";
		  ...
		  if(strcmp($vars['enable_ticket_status'], 'on') == 0){
          	return $this->updateAll(array(
				...
				'mod_status_default_shape'=>$vars['default_shape'],
				'mod_status_display_text'=>$vars['display_option'],
		 	));
		  }else{
			return $this->updateAll(array(
            	'mod_status_enabled'=>$vars['enable_ticket_status'],
