    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
		...
		switch (strtolower($origin)) {
            case 'staff':
				...
				if(!$res = db_query($sql)){
					$errors['err'] = 'Unable to add new ticket assignment->'.$sql;	
				}
				break;
			default:
				$sql = "INSERT INTO ".MOD_STATUS_ASSIGNMENTS." (ticketId, statusId, assignerId) VALUES (".$id.", ". $cfg->get('mod_status_default_status').", NULL)";
				if(!$res = db_query($sql)){
					$errors['err'] = 'Unable to add new ticket assignment->'.$sql;	
				}
				break;
